...
import { useTryOnTabStore } from "@/stores/useTryOnTabStore";
...
const TopsPage = memo(() => {
  const { t } = useTranslation(["try_on", "common"]);
  ...
  const builder = supabase
    ...
    .is("feedback", null)
    .or("t_vton.tops_id.not.is.null,t_vton.dress_id.not.is.null")
    .order("created_at", { ascending: true })
    .order("id", { ascending: true })
    .limit(3);

  const { mutate } = useQuery(builder, {
    onSuccess: ({ data }) => {
      if (!data) {
        return;
      }
  ...
  useEffect(() => {
    // mark this screen as active tab
    useTryOnTabStore.getState().setTab("tops");
